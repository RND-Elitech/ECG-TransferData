/**
 * @file web_server.c
 * @brief HTTP Web Server untuk Captive Portal / Configuration Portal ECG
 * Gateway
 *
 * Endpoint:
 ...
 */
static bool parse_form_field(const char *body, const char *key, char *out_buf,
                             size_t out_len) {
 ...
 */
static esp_err_t nvs_set_str_helper(nvs_handle_t h, const char *key,
                                    const char *val) {
...
static esp_err_t handler_save(httpd_req_t *req) {
  ...
  /* Validasi minimal */
  if (wifi_ssid[0] == '\0') {
    const char *resp = "{\"status\":\"error\",\"message\":\"WiFi Name (SSID) "
                       "cannot be empty\"}";
    httpd_resp_set_type(req, "application/json");
  ...
  if (strlen(wifi_pass) < 8) {
    const char *resp = "{\"status\":\"error\",\"message\":\"Password must be "
  ...
  // Gunakan timeout 30 detik (30000 ms) untuk pengujian
  esp_err_t conn_err = wifi_manager_start(wifi_ssid, wifi_pass, 30000);
  ...
  if (conn_err != ESP_OK) {
    ...
    snprintf(resp_err, sizeof(resp_err),
             "{\"status\":\"error\",\"message\":\"Failed to connect to WiFi "
             "'%s'. Incorrect password or network not found.\"}",
             wifi_ssid);
  ...
  if (err != ESP_OK) {
    ...
    const char *resp = "{\"status\":\"error\",\"message\":\"Failed to save "
                       "settings (NVS error)\"}";
    httpd_resp_set_type(req, "application/json");
    httpd_resp_sendstr(req, resp);
    return ESP_OK;
  ...
  if (ip_addr[0])
    nvs_set_str_helper(nvs_h, "ip_addr", ip_addr);
  if (subnet[0])
    nvs_set_str_helper(nvs_h, "ip_subnet", subnet);
  if (gateway[0])
    nvs_set_str_helper(nvs_h, "ip_gw", gateway);
  ...
  const char *resp_ok = "{\"status\":\"success\","
                        "\"message\":\"Configuration saved! The device will "
                        "restart and connect to the WiFi network...\"}";
  httpd_resp_set_type(req, "application/json");
...
static esp_err_t handler_ap_status(httpd_req_t *req) {
  char resp[64];
  snprintf(resp, sizeof(resp), "{\"remaining_seconds\":%ld}",
           (long)g_ap_timeout_sec);
  httpd_resp_set_type(req, "application/json");
  httpd_resp_sendstr(req, resp);
  return ESP_OK;
}
...
static esp_err_t handler_ota_check(httpd_req_t *req) {
  ...
  if (!wifi_manager_is_connected()) {
    ...
    cJSON_AddStringToObject(root, "current_version",
    ...
    cJSON_AddStringToObject(
        root, "error",
        "Perangkat tidak terhubung ke internet. Hubungkan ke WiFi terlebih "
        "dahulu untuk memeriksa pembaruan.");
...
esp_err_t web_server_start(void) {
  ...
  config.stack_size = 10240; /* Tambah stack size untuk mencegah overflow saat
                                OTA check (TLS/HTTPS) */
  ...
  static const httpd_uri_t uris[] = {
      ...
      {.uri = "/ota/progress",
       .method = HTTP_GET,
       .handler = handler_ota_progress},
      {.uri = "/generate_204",
       .method = HTTP_GET,
       .handler = handler_generate_204},
...
void web_server_set_dashboard_mode(bool enable) {
  ...
  ESP_LOGI(TAG, "Dashboard mode: %s",
           enable ? "ON (dashboard.html)" : "OFF (root.html)");
}

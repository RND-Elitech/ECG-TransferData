...
static void _ota_update_task(void *pvParameter) {
  ...
  if (err != ESP_OK) {
    snprintf(s_error_msg, sizeof(s_error_msg),
             "OTA begin gagal setelah 3x percobaan: %s", esp_err_to_name(err));
  ...
  if (err != ESP_OK) {
    snprintf(s_error_msg, sizeof(s_error_msg), "OTA perform gagal: %s",
             esp_err_to_name(err));
    ESP_LOGE(TAG, "%s", s_error_msg);
  ...
  if (err != ESP_OK) {
    snprintf(s_error_msg, sizeof(s_error_msg), "OTA finish gagal: %s",
             esp_err_to_name(err));
    ESP_LOGE(TAG, "%s", s_error_msg);
    s_state = OTA_STATE_FAILED;
    vTaskDelete(NULL);
...
esp_err_t ota_manager_check(ota_check_result_t *result) {
  ...
  strncpy(result->current_version, APP_VERSION,
          sizeof(result->current_version) - 1);

  /* Fetch version.json */
  ESP_LOGI(TAG, "OTA CHECK URL: '%s'", OTA_VERSION_URL);
  ...
  esp_http_client_config_t http_cfg = {
      ...
      .crt_bundle_attach =
          esp_crt_bundle_attach, /* Gunakan global certificate bundle */
  };
  ...
  /* Supabase REST API (PostgREST) mengembalikan Array JSON, ambil elemen
   * pertama */
  ...
  if (!cJSON_IsString(j_version) || !cJSON_IsString(j_url)) {
    ESP_LOGE(TAG, "Format data Supabase tidak valid (butuh 'version' dan "
                  "'firmware_url')");
    cJSON_Delete(root);
    s_state = OTA_STATE_IDLE;
    return ESP_FAIL;
  ...
  strncpy(result->latest_version, j_version->valuestring,
          sizeof(result->latest_version) - 1);
  strncpy(result->firmware_url, j_url->valuestring,
          sizeof(result->firmware_url) - 1);
  if (cJSON_IsString(j_notes)) {
    strncpy(result->release_notes, j_notes->valuestring,
            sizeof(result->release_notes) - 1);
  }

  result->update_available =
      (_compare_versions(result->latest_version, result->current_version) > 0);
  cJSON_Delete(root);
